...
app.use((req, res, next) => {
  ...
  res.on("finish", () => {
    ...
    if (path.startsWith("/api")) {
      ...
      if (capturedJsonResponse && !path.includes("provider-config") && !path.includes("auth")) {
        const safeLog = { ...capturedJsonResponse };
        delete safeLog.skillCode;
        delete safeLog.apiKey;
        delete safeLog.secretAccessKey;
        delete safeLog.accessKeyId;
        logLine += ` :: ${JSON.stringify(safeLog).slice(0, 200)}`;
      }
